"""
Configure System to Use ONLY WebShare Premium Proxies - SIMPLE VERSION
Uses direct database queries to avoid caching issues
"""
import os
import sys
import asyncio
...
async def configure_webshare_only():
    ...
    webshare_proxies = cursor.fetchone()[0]
    ...
    if free_proxies > 0:
        ...
        cursor.execute("DELETE FROM proxy_pool WHERE provider != 'webshare' OR provider IS NULL")
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM proxy_pool WHERE provider = 'webshare'")
    current_webshare = cursor.fetchone()[0]
    conn.close()
    ...
    if current_webshare == 0:
        ...
        result = await webshare.import_to_database(max_proxies=proxy_count)
        ...
        if result.get('status') == 'success':
            ...
            logger.error(f"   ❌ Failed to fetch WebShare proxies: {result.get('error')}")
            return False
    else:
    ...
    cursor.execute("SELECT COUNT(*) FROM proxy_pool WHERE provider = 'webshare' AND is_active = 1")
