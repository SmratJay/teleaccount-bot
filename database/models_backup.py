...
class TelegramAccount(Base):
    ...
    frozen_by_admin_id = Column(Integer, ForeignKey('users.id'), nullable=True)
    freeze_timestamp = Column(DateTime, nullable=True)
    freeze_duration_hours = Column(Integer, nullable=True)  # None = indefinite
    can_be_sold = Column(Boolean, default=True)  # False when frozen or on hold
    ...
    # Relationships
    seller = relationship("User", back_populates="accounts", foreign_keys=[seller_id])
    frozen_by_admin = relationship("User", foreign_keys=[frozen_by_admin_id])
    sales = relationship('AccountSale', back_populates='account', cascade='all, delete-orphan')
    sale_logs = relationship('AccountSaleLog', back_populates='account', cascade='all, delete-orphan')
...
class SaleLogStatus(enum.Enum):
    ...
    APPROVED = "APPROVED"        # Admin approved the sale
    REJECTED = "REJECTED"        # Admin rejected the sale
    AUTO_APPROVED = "AUTO_APPROVED"  # Automatically approved (no issues)

class AccountSaleLog(Base):
    __tablename__ = 'account_sale_logs'
    
    id = Column(Integer, primary_key=True)
    sale_id = Column(Integer, ForeignKey('account_sales.id'), nullable=False)
    account_id = Column(Integer, ForeignKey('telegram_accounts.id'), nullable=False)
    seller_id = Column(Integer, ForeignKey('users.id'), nullable=False)
    ...
    account_status = Column(String(50), nullable=True)  # Status at time of sale
    
    # Review information
    log_status = Column(Enum(SaleLogStatus), default=SaleLogStatus.PENDING)
    ...
    # Additional metadata
    is_account_frozen = Column(Boolean, default=False)
    multi_device_detected = Column(Boolean, default=False)
    multi_device_last_detected = Column(DateTime, nullable=True)
    security_flags = Column(Text, nullable=True)  # JSON string with security issues
    
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    sale = relationship("AccountSale", foreign_keys=[sale_id])
    ...
    def __repr__(self):
        return f"<AccountSaleLog(id={self.id}, sale_id={self.sale_id}, status='{self.log_status}')>"
